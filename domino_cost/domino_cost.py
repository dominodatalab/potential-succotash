...
def distribute_cost(df: DataFrame) -> DataFrame:
    ...
    """
    fields_list = [CostFieldsLabels.CPU_COST, CostFieldsLabels.GPU_COST, CostFieldsLabels.COMPUTE_COST, CostFieldsLabels.MEMORY_COST, CostFieldsLabels.STORAGE_COST, CostFieldsLabels.ALLOC_COST]
...
def get_cumulative_cost_graph(cost_table: DataFrame, time_span: timedelta):
    x_date_series = pd.date_range(get_today_timestamp() - get_time_delta(time_span), get_today_timestamp()).strftime(
        '%B %-d')
...
def workload_cost_details(cost_table: DataFrame):
    ...
    table = dash_table.DataTable(
        columns=[
            ...
            {'name': CostFieldsLabels.CPU_COST, 'id': CostFieldsLabels.CPU_COST, 'type': 'numeric', 'format': formatted},
            {'name': CostFieldsLabels.GPU_COST, 'id': CostFieldsLabels.GPU_COST, 'type': 'numeric', 'format': formatted},
            {'name': CostFieldsLabels.STORAGE_COST, 'id': CostFieldsLabels.STORAGE_COST, 'type': 'numeric', 'format': formatted},
            {'name': CostAggregatedLabels.CLOUD_COST, 'id': CostAggregatedLabels.CLOUD_COST, 'type': 'numeric', 'format': formatted},
            {'name': CostAggregatedLabels.TOTAL_COST, 'id': CostAggregatedLabels.TOTAL_COST, 'type': 'numeric', 'format': formatted},
        ],
...
def get_dropdown_filters(cost_table: DataFrame) -> tuple:
    ...
    projects_list = clean_values(sorted(cost_table[CostLabels.PROJECT_NAME].unique().tolist(), key=str.casefold))
    ...
    billing_tags_dropdown = [{"label": billing_tag, "value": billing_tag, "title": billing_tag} for billing_tag in
                             billing_tags_list]
...
def build_histogram(cost_table: DataFrame, bin_by: str):
    ...
    chart = px.histogram(costs, x=CostAggregatedLabels.TOTAL_COST, y=bin_by, orientation='h',
                         title=title, labels={bin_by: bin_by.title(), CostAggregatedLabels.TOTAL_COST: 'Total Cost'},
    ...
    chart.update_layout(
        ...
        yaxis={  # Trim labels that are larger than 15 chars
            'tickmode': 'array',
            'tickvals': data_index,
            'ticktext': [f"{txt[:15]}..." if len(txt) > 15 else txt for txt in
                         chart['layout']['yaxis']['categoryarray']]
        },
        dragmode=False
    )
